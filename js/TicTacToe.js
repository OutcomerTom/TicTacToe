...
const boxClicked = (e) => {
  ...
  if (!spaces[id]) {
    ...
    myColorChanges(e.target, currentPlayer);
    currentPlayer = currentPlayer === tick_circle ? tick_x : tick_circle;
  }
};

const myColorChanges = (target, currentPlayer) => {
  if (currentPlayer === tick_circle) {
...
const playerDraw = () => {
  let draw = 0;
  spaces.forEach((_space, i) => {
    if (spaces[i] !== null) draw++;
  });
  if (draw === 9) {
    text.innerText = `Cats game!`;
    restart();
  }
};
